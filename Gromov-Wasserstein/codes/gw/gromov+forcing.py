...
def gromov_wasserstein(C1,C2,p,q,L,epsilon,numItermax = 1000, stopThr=1e-9,verbose=False, log=False,forcing=[]):
    """
    ...
    while (err>stopThr and cpt<numItermax):
        ...
        tens=L(C1,C2,T)
        if len(forcing)!=0:
            for couple in forcing:
                tens[couple[0],couple[1]]=0
...
def class_gromov_wasserstein(C1,C2,p,q,L,epsilon,class_forcing,numItermax = 1000, stopThr=1e-9,verbose=False, log=False):
    """
    Returns the gromov-wasserstein discrepancy between the two measured similarity matrices
